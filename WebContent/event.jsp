<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="utf-8"%>
    <%@page import="com.aleria.entities.*" %>
    <%@page import="java.util.List" %>
    <%@page import="java.util.ArrayList" %>
    <%List<User> listAccepted = new ArrayList<User>() ;
    	List<User> listDeclined = new ArrayList<User>() ;
    	List<User> listWaiting = new ArrayList<User>() ;
    	listAccepted = (ArrayList<User>) request.getAttribute("listAccepted");
    	listDeclined = (ArrayList<User>) request.getAttribute("listDeclined");
    	listWaiting = (ArrayList<User>) request.getAttribute("listWaiting"); 
    	Event event = new Event() ; 
    	event = (Event) request.getAttribute("eventDetails");
    	Host host = new Host() ; 
    	host = (Host) request.getAttribute("host"); 
    	%>	
<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
<title>Détails de l'événement</title>
    <%@include file="ressource.html" %>
    <link rel="stylesheet" href="css/event.css"/>


</head>
<body>
<%@include file="header.html" %>
<main>
<div class="page-event">
  <div class="container">
    <div class="upcoming-sec">
      <div class="heading">Les événements à venir</div>
    </div>
    <div class="upcoming-event-list">
      <div class="event-block">
        <div class="row"  style="flex-wrap: nowrap !important">
          <div class="col-lg-2 sec-1">
            <table>
              <tr>
                <td>
                  <div class="month"><%= event.getEventDate().getMonth() %></div>
                  <div class="month-date-devider"></div>
                  <div class="date"><%=event.getEventDate().getDayOfMonth() %></div>
                </td>
                <td class="title"></td>
              </tr>
            </table>
          </div>
          <div class="col-lg-5  sec-2">
            <img style="padding-left:10% ;padding-right:10%; margin-top: 10%" src=<%if(event.getEventType().equals("Sortie")) { out.println("img/outing.webp"); } else { out.println("img/inside.webp");} %> />
          </div>
          <div class="col-lg-5 sec-3">
            <div class="title"><%= event.getEventTitle() %></div>
            <div class="venue">
              <table>
                <tr>
                  <td><i class="fa fa-address-book"></i></td>
                  <td>
                    <div><a href="profile?idUser=<%=host.getIdUser()%>"><%=host.getUserName() %></a></div>
                    <div class="dim-color">
                      <i class="fa fa-map-marker"></i> <%= event.getAddress() %>
                    </div>
                  </td>
                </tr>
              </table>
            </div>
            <div class="time">
              <table>
                <tr>
                  <td><i class="fa fa-clock-o"></i></td>
                  <td>
                    <div><%=event.getEventDate() %></div>
                    <div data-livestamp="1517054400" class="dim-color"></div>
                  </td>
                </tr>
              </table>
            </div>
            <div class="sort-story">"<%=event.getDescription() %>" - <%= host.getUserName() %></div>
            
          </div>
        </div>
      </div>
    </div>
  </div>
</div>
<div class="X">
<ul class="SG">
  <li class="sgLi">
    <div class="box">
    <h3>Ont accepté</h3>
    <ul class="df">
    <%for (User u:listAccepted) {%>
    <li><a href="profile?idUser=<%=u.getIdUser() %>"><%=u.getUserName() %></a></li><%} %>
    </ul> 
  </div>
 </li> 
 
   <li class="sgLi">
    <div class="box">
    <h3>En attente</h3>
    <ul class="df">
    <%for (User u:listWaiting){ %>
    <li><a href="profile?idUser=<%=u.getIdUser() %>"><%=u.getUserName() %></a><li><%} %>
    </ul> 
  </div>
 </li> 
 
   <li class="sgLi">
    <div class="box">
    <h3>Ont refusé</h3>
    <ul class="df">
    <%for (User u:listDeclined) {%>
    <li><a href="profile?idUser=<%=u.getIdUser() %>"><%=u.getUserName() %></a></li><%} %>
    </ul> 
  </div>
 </li> 
</ul>
</div>

</main>
<%@include file="footer.html" %>
</body>
</html>